...
(function (jcast) {
    var Asset = /** @class */ (function () {
        function Asset(_a) {
            var _b = _a.data, data = _b === void 0 ? { displayLogs: true, sections: [] } : _b;
            this._data = { displayLogs: true, sections: [] };
            this._assets = {};
            this._data = data;
        }
        ...
        Asset.prototype.load = function (callback) {
            ...
            if (this._data.displayLogs) {
                console.log(jcast.JCast.getIdentifier() + ": Loading game assets...");
            }
            for (var _i = 0, _a = this._data.sections; _i < _a.length; _i++) {
                ...
                for (var _b = 0, _c = section.resources; _b < _c.length; _b++) {
                    ...
                    if (this._assets[section.name][resource.name] != undefined) {
                        loaded++;
                        if (this._data.displayLogs) {
                            console.warn(jcast.JCast.getIdentifier() + ": Skipping already loaded resource");
                        }
                        continue;
                    }
                    switch (resource.type) {
                        case 'image':
                            ...
                            image.onload = function (e) {
                                ...
                                if (loaded == total) {
                                    ...
                                    if (_this._data.displayLogs) {
                                        console.log(jcast.JCast.getIdentifier() + ": All assets loaded correctly");
                                    }
                                }
                            };
                        ...
                        case 'color':
                            ...
                            jcast.http.get(resource.url, params, function (params, data) {
                                var color = new jcast.Color(data);
                                _this._assets[params.section][params.resource] = color;
                                loaded++;
                                if (loaded == total) {
                                    if (callback) {
                                        callback(loaded, total);
                        ...
                        default:
                            loaded++;
                            if (this._data.displayLogs) {
                                console.warn(jcast.JCast.getIdentifier() + ": Resource " + resource.name + " has an invalid type");
                            }
                            break;
                    }
                }
        ...
        Asset.prototype.getAssetsCount = function () {
            ...
            this._data.sections.forEach(function (section) {
                section.resources.forEach(function () {
                    total++;
                });
            });
            return total;
